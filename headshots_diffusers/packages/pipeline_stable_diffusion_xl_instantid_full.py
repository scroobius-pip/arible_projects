...
class LongPromptWeight(object):
    ...
    def get_prompts_tokens_with_weights(
        self, clip_tokenizer: CLIPTokenizer, prompt: str
    ):
        """
    ...
    def group_tokens_and_weights(
        self, token_ids: list, weights: list, pad_last_block=False
    ):
        """
    ...
    def get_weighted_text_embeddings_sdxl(
        ...
        # tokenizer 1
        prompt_tokens, prompt_weights = self.get_prompts_tokens_with_weights(
            pipe.tokenizer, prompt
        )
        neg_prompt_tokens, neg_prompt_weights = self.get_prompts_tokens_with_weights(
        ...
        prompt_tokens_2, prompt_weights_2 = self.get_prompts_tokens_with_weights(
            pipe.tokenizer, prompt
        )
        neg_prompt_tokens_2, neg_prompt_weights_2 = (
            self.get_prompts_tokens_with_weights(pipe.tokenizer, neg_prompt)
        )
        ...
        for i in range(len(prompt_token_groups)):
            ...
            neg_token_embedding = torch.concat(neg_prompt_embeds_list, dim=-1).squeeze(
                0
            )
        ...
        if extra_emb is not None:
            extra_emb = (
                extra_emb.to(prompt_embeds.device, dtype=prompt_embeds.dtype)
                * extra_emb_alpha
            )
            prompt_embeds = torch.cat([prompt_embeds, extra_emb], 1)
            negative_prompt_embeds = torch.cat(
                [negative_prompt_embeds, torch.zeros_like(extra_emb)], 1
            )
    ...
    def get_prompt_embeds(self, *args, **kwargs):
        prompt_embeds, negative_prompt_embeds, _, _ = (
            self.get_weighted_text_embeddings_sdxl(*args, **kwargs)
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if control_mask is not None:
            ...
            mask_weight_image_tensor = torch.from_numpy(mask_weight_image).to(
                device=device, dtype=prompt_embeds.dtype
            )
            mask_weight_image_tensor = mask_weight_image_tensor[:, :, 0] / 255.0
            ...
            region_mask = (
                torch.from_numpy(np.array(control_mask)[:, :, 0]).to(
            ...
            for i, t in enumerate(timesteps):
                ...
                if isinstance(self.controlnet, MultiControlNetModel):
                    ...
                    for control_index in range(len(self.controlnet.nets)):
                        ...
                        if (
                            control_index == 0
                            and control_mask_wight_image_list is not None
                        ):
                            down_block_res_samples = [
                                down_block_res_sample * mask_weight
                                for down_block_res_sample, mask_weight in zip(
                                    down_block_res_samples,
                                    control_mask_wight_image_list,
                                )
                            ]
                    ...
                    mid_block_res_sample = torch.stack(mid_block_res_sample_list).sum(
                        dim=0
                    )
                    down_block_res_samples = [
                    ...
                    if control_mask_wight_image_list is not None:
                        down_block_res_samples = [
                            down_block_res_sample * mask_weight
                            for down_block_res_sample, mask_weight in zip(
                                down_block_res_samples, control_mask_wight_image_list
                            )
                        ]
                        mid_block_res_sample *= control_mask_wight_image_list[-1]
